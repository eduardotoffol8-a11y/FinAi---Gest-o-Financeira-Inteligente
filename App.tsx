...
function App() {
  ...
  const NavItem = ({ icon: Icon, label, active, onClick }: any) => (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-6 py-5 rounded-[1.5rem] text-sm font-black transition-all duration-300 relative group ${
        active ? 'bg-slate-900 text-white shadow-2xl translate-x-1' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-900'
      }`}
    >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-24 bg-white/80 backdrop-blur-2xl border-b border-slate-100 flex items-center justify-between px-6 lg:px-12 z-[140]">
          ...
          <div className="flex items-center gap-6">
             ...
             <div className="w-12 h-12 rounded-[1.25rem] bg-slate-950 flex items-center justify-center text-white font-black italic shadow-2xl border-4 border-white">
                {user.name.charAt(0)}
             </div>
          </div>
        </header>
        ...
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-10 border border-white/20">
             <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black text-slate-900 italic">Compartilhar</h3>
                <button onClick={() => setSharingModal({ open: false, item: null, type: 'transaction' })} className="p-2 hover:bg-slate-100 rounded-full"><X className="w-6 h-6 text-slate-400"/></button>
             </div>
             <div className="space-y-3">
                {team.filter(m => m.id !== user.id).map(member => (
                   <button 
                      ...
                      className="w-full flex items-center justify-between p-4 bg-slate-50 hover:bg-indigo-50 rounded-2xl border border-slate-200 transition-all"
                   >
